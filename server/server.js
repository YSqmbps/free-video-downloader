...
const mockVideoInfo = {
  ...
  formats: [
    { format_id: '1', ext: 'mp4', width: 1920, height: 1080, format_note: '1080p', filesize: 104857600, format: '1080p MP4' },
    { format_id: '2', ext: 'mp4', width: 1280, height: 720, format_note: '720p', filesize: 52428800, format: '720p MP4' },
    { format_id: '3', ext: 'mp4', width: 854, height: 480, format_note: '480p', filesize: 26214400, format: '480p MP4' },
    { format_id: '4', ext: 'mp4', width: 640, height: 360, format_note: '360p', filesize: 13107200, format: '360p MP4' },
    { format_id: '5', ext: 'mp3', format_note: 'audio', filesize: 5242880, format: 'Audio MP3' },
  ]
};
  ...
  try {
    ...
    const request = protocol.get(url, options, (response) => {
      ...
      if (response.statusCode !== 200) {
        ...
        res.status(500).json({ error: `Failed to fetch image, status: ${response.statusCode}` });
        return;
      }

      const contentType = response.headers['content-type'] || 'image/jpeg';
      const contentLength = response.headers['content-length'];
      console.log(`[Proxy Image] Content-Type: ${contentType}, Content-Length: ${contentLength}`);
...
app.post('/api/download', async (req, res) => {
  ...
  try {
    ...
    const ffmpegPath = findFfmpegSync();
    if (!ffmpegPath) {
      return res.status(500).json({ error: 'FFmpeg 不可用，请确保已安装 FFmpeg' });
    }
    
    let formatArg;
    let args = ['-o', outputPath, '--no-warnings', '--progress', '--newline', '-v'];

    if (format === 'mp3') {
      formatArg = 'bestaudio/best';
      args.push('-f', formatArg, url);
    } else {
      formatArg = 'bestvideo+bestaudio/best';
      args.push('-f', formatArg, '--merge-output-format', 'mp4', '--ffmpeg-location', ffmpegPath, url);
      console.log(`[Download] Using ffmpeg at ${ffmpegPath} to merge video+audio, format: ${formatArg}`);
    }
    ...
    ytdlp.stdout.on('data', (data) => {
      ...
      for (const line of lines) {
        const downloadProgressMatch = line.match(/\[download\]\s*(\d+\.?\d*)%\s+of/);
    ...
    ytdlp.stderr.on('data', (data) => {
      ...
      for (const line of lines) {
        ...
        if (line.includes('Post-processing')) {
          sendProgress({ status: 'processing', progress: 98 });
        }
      }
    });
    ...
    ytdlp.on('close', (code) => {
      if (code === 0) {
        ...
        const downloadedFile = files.find(file => file.startsWith(videoId) && !file.endsWith('.jpg') && !file.endsWith('.webp') && !file.endsWith('.png'));
...
app.post('/api/info', async (req, res) => {
  ...
  try {
    ...
    ytdlp.on('close', (code) => {
      ...
      if (timeout) {
        return res.status(500).json({ error: 'Request timed out', details: 'yt-dlp took too long to respond' });
      }
      ...
      if (code === 0) {
        (async () => {
          ...
          console.log('Received info:', JSON.stringify({
            hasTitle: !!info.title,
          ...
          const formats = info.formats
            ? info.formats
                .filter(f => f.ext && ['mp4', 'mp3', 'webm', 'mkv'].includes(f.ext))
                .map(f => ({
                  id: f.format_id || Math.random().toString(36).substr(2, 9),
                  format: f.format || 'unknown',
                  resolution: getResolution(f),
          ...
          console.log('Formats extracted:', formats.length);

          const thumbnailUrl = getThumbnail(info);
          const base64Thumbnail = thumbnailUrl ? await fetchThumbnailAsBase64(thumbnailUrl) : null;
